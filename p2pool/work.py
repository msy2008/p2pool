    ...
    def get_work(self, pubkey_hash, desired_share_target, desired_pseudoshare_target):
        ...
        if desired_share_target is None:
            ...
            if previous_share is not None and self.node.tracker.get_height(previous_share.hash) > lookbehind:
                ...
                if expected_payout_per_block < self.node.net.PARENT.DUST_THRESHOLD and block_subsidy != 0:
                    desired_share_target = min(desired_share_target,bitcoin_data.average_attempts_to_target((bitcoin_data.target_to_average_attempts(self.node.bitcoind_work.value['bits'].target)*self.node.net.SPREAD)*self.node.net.PARENT.DUST_THRESHOLD/block_subsidy))
